...
describe("MaintenanceView:", () => {
  ...
  it("renders twitter and uptime links", async () => {
    ...
    expect(wrapper.findAll(`.description a`)[0].attributes("href")).toBe("https://twitter.com/PlayFiGaming");
    expect(wrapper.findAll(`.description a`)[1].attributes("href")).toBe("https://uptime.com/s/zkSync-testnet");
    expect(wrapper.find(`.twitter-button`).attributes("href")).toBe("https://twitter.com/PlayFiGaming");
